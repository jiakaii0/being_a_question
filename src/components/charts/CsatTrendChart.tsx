...
export default function CsatTrendChart({ current, previous }: Props) {
  ...
  // Merge current + previous into a unified dataset indexed by position
          ...
          {(['wow', 'mom'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)}
              className={`px-3 py-1 text-xs font-medium rounded transition ${
                mode === m ? 'bg-shopee-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}>
              {m === 'wow' ? 'Week on Week' : 'Month on Month'}
            </button>
          ))}
        </div>
      </div>
