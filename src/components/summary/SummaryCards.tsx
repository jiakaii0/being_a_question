...
export default function SummaryCards({ summary }: Props) {
  ...
  const trendColor = csatDelta > 0 ? 'text-green-600' : csatDelta < 0 ? 'text-red-500' : 'text-gray-400'
      ...
      {cards.map((card, i) => (
        <Card key={i} className="border border-gray-200 shadow-sm">
          <CardHeader className="pb-1 pt-4 px-4">
            <CardTitle className="text-xs font-medium text-gray-500 uppercase tracking-wider flex items-center gap-1">
          ...
          <CardContent className="px-4 pb-4">
            {card.badge !== undefined ? (
              ...
              <p className={`text-3xl font-extrabold ${card.valueClass ?? 'text-gray-800'}`}>{card.value}</p>
            )}
